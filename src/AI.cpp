...
AI::AI()
{
    ...
    m_EnemyShape.setPosition(-100, -100); //sets the dafault enemy out of the vision of the player, couldn't get it removed
}
...
void AI::UpdateAI1()
{
 int moveai = rand()%100;
    ...
    {
        m_EnemyShape.move(0.0f, -25.0f);//up
        lastDirection = 0;
    }
    if(moveai == 1 && lastDirection != 0)
    {
        m_EnemyShape.move(0.0f, 25.0f);//down
        lastDirection = 1;
    }
    if(moveai == 2 && lastDirection != 3)
    {
        m_EnemyShape.move(-25.0f, 0.0f);//left
        lastDirection = 2;
    }
    if(moveai == 3 && lastDirection != 2)
    {
        m_EnemyShape.move(25.0f, 0.0f);//right
        lastDirection = 3;
    }
}
